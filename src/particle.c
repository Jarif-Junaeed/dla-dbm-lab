...
void Walk(struct clusterDeltas *cluster_deltas, struct stats *cluster_stats, struct point p, size_t cluster_center, double *radius, bool *cluster, pcg32_random_t *rng) {
  for (size_t i = 0; i < MAX_PARTICLE_STEPS; i++) {
    ...
    // Particle went out of bounds
    if (next_x < 0 || next_x >= WIDTH || next_y < 0 || next_y >= HEIGHT) {
      cluster_deltas->m_ReduceClusterDeltasUsedCapacity(cluster_deltas, i+1);
      return;
    }
    ...
    // if particle does not stick to cluster within MAX_PARTICLE_STEPS, then we kill it
    else if ( i == MAX_PARTICLE_STEPS - 1) {
      cluster_deltas->m_ReduceClusterDeltasUsedCapacity(cluster_deltas, i+1);
      return;
    }
